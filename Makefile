...
ldflags= -X github.com/uplo-tech/uplomux/build.GitRevision=${GIT_DIRTY}${GIT_REVISION} \
-X "github.com/uplo-tech/uplomux/build.BuildTime=${BUILD_TIME}"
...
test-long: clean fmt vet lint-ci
	@mkdir -p cover
	go test --coverprofile='./cover/cover.out' -v -race -failfast -tags='testing debug netgo' -timeout=1800s $(pkgs) -run=$(run) -count=$(count)
